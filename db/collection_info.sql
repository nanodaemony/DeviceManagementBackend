...
create TABLE `info_medical_device` (
    ...
    `device_type` VARCHAR(32) NOT NULL COMMENT '仪器类型(大类：麻醉机、呼吸机、监护仪等),如果涉及多个类别存成字符串形式,如1#2#5',
...
CREATE TABLE `info_device_usage_evaluation` (
    ...
    `collection_number` INT NOT NULL COMMENT '采集场次号',
    `unique_number`        VARCHAR(32) NOT NULL COMMENT '唯一序列号,由采集场次号+产生时时间戳组成',
...
CREATE TABLE `info_device_maintenance_record` (
    ...
    `maintenance_mode` VARCHAR(16) COMMENT '维修方式:自修 上门修 返厂修 第三方维修 其他',
    `maintenance_people` VARCHAR(32) COMMENT '维修人员: 设备科维修人员 厂家维修人员 科室配备维修人员 设备使用人员 其他',
    `within_the_shelf_life` BOOL COMMENT '是否在保质期内: 是/否',
    `error_reason` VARCHAR(64) COMMENT '故障原因: 配件损坏 软件故障 操作失误 环境因素 其他',
    ...
    `error_not_overcome_reason` VARCHAR(64)  DEFAULT NULL COMMENT '故障未解决原因',
