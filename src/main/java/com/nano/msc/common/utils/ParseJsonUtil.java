...
public class ParseJsonUtil {
    ...
    public static <T> T parseAndCheck(String jsonData, Class<T> tClass) {
        ...
        try {
            ...
            ExceptionAsserts.fail(ExceptionEnum.DATA_PARSE_EXCEPTION, jsonData);
        }

        // 进行数据校验
        Map<String, String> validResult = ValidatorUtils.validate(t);
        if (Objects.nonNull(validResult) && !validResult.isEmpty()) {
            ExceptionAsserts.fail(ExceptionEnum.DATA_PARSE_EXCEPTION, JSON.toJSONString(validResult));
        }
        return t;
    }
